...
def main():
    ...
    model.build()
    model.restore_session(config.dir_model + config.run_name + '/model.ckpt')

    elmo_model = get_elmo_model(config.filename_chars, config.elmo_option_file, config.elmo_file,
                                config.elmo_token_embedding_file)

    # create dataset
    test = CoNLLDataset(config.filename_test, elmo_model, config.elmo_dim,
