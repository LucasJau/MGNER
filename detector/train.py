from dtc_model.data_utils import CoNLLDataset, get_elmo_model
from dtc_model.ner_model import NERModel
from dtc_model.config import Config
import argparse
import os
import time
...
# parser.add_argument("--hidden_size_lstm_1", type=int, default=100, help="lstm_1 hidden dim", dest='hidden_size_lstm_1')
# parser.add_argument("--hidden_size_lstm_2", type=int, default=100, help="lstm_1 hidden dim", dest='hidden_size_lstm_2')
...
# parser.add_argument("--batch_size", type=int, default=32, help="batch_size", dest='batch_size')
...
def main():
    ...
    elmo_model = get_elmo_model(config.filename_chars, config.elmo_option_file, config.elmo_file, config.elmo_token_embedding_file)

    # create datasets
    dev = CoNLLDataset(config.filename_dev, elmo_model, config.elmo_dim,
                       config.filename_chars, config.generate_anchor, config.max_iter)
    train = CoNLLDataset(config.filename_train, elmo_model, config.elmo_dim,
                         config.filename_chars, config.generate_anchor, config.max_iter)
