...
from cls_model.utils import report
from cls_model.data_utils import minibatches, pad_sequences
from cls_model.general_utils import Progbar
from cls_model.base_model import BaseModel
...
class NERModel(BaseModel):
    ...
    def add_placeholders(self):
        ...
        self.anchor_features = tf.placeholder(tf.float32,
                shape=[None, self.config.anchor_types, 100],
                name="anchor_features")

        # shape = (batch size, max_anchor_len, anchor_feature_len)
        self.elmo_features = tf.placeholder(tf.float32,
                shape=[None, self.config.anchor_types, self.config.elmo_dim],
                name="elmo_features")
        ...
        self.word_lengths = tf.placeholder(tf.int32, shape=[None],
                       name="word_lengths")
    ...
    def get_feed_dict(self, features, elmo_features, context_features,
        ...
        all_context, all_context_len = pad_sequences(
                all_context, pad_tok=[0]*2*self.config.hidden_size_lstm_1) # 600
        ...
        with tf.variable_scope("chars"): # chars
            if self.config.use_chars:
                ...
                _output = tf.nn.bidirectional_dynamic_rnn(
                        ...
                        sequence_length=self.word_lengths, dtype=tf.float32)
    ...
    def add_elmo_op(self):
        with tf.variable_scope("elmo"):
            ...
            self.concat_hidden = tf.concat([self.elmo_features, self.lstm_1_output], axis=-1)
    ...
    #         cell_fw = tf.contrib.rnn.LSTMCell(100)
    #         cell_bw = tf.contrib.rnn.LSTMCell(100)
    #         _, (output_states_fw, output_states_bw) = tf.nn.bidirectional_dynamic_rnn(
    ...
    def add_context_att_lstm_op(self):
        with tf.variable_scope("context_att_lstm"): # bi-lstm
            ...
            self.WA = tf.get_variable("WA", dtype=tf.float32, shape=[2*self.config.hidden_size_lstm_2, 2*self.config.hidden_size_lstm_2],
            ...
            expand_attention = tf.tile(tf.expand_dims(self.attention, -1), [1, 1, 2*self.config.hidden_size_lstm_2])
            ...
            self.context_output = tf.concat([output_states_fw[0], output_states_bw[0]], axis = -1)
            self.last_output = tf.concat([self.context_output, self.last_output], axis=-1)
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("cls"):
            W1 = tf.get_variable("W1", dtype=tf.float32,
                    shape=[2*2*self.config.hidden_size_lstm_2, self.config.cls_hidden_size])
    ...
    def run_epoch(self, train, dev, epoch, dev_total_entity):
        ...
        for i, (features, elmo_features, lens, labels, char_ids, word_lens, context_features, lc, ll, rc, rl) in enumerate(minibatches(train, batch_size)):

            fd = self.get_feed_dict(features, elmo_features, context_features,
                    lc, ll, rc, rl, char_ids,
                    word_lens, lens = lens, labels = labels,
